...
function simulate()
    current_call_durations = [call_duration() for _ in 1:5] # initialize: all workers start a call at t=0

    event_list = copy(current_call_durations)
    revenue = 0.0
    t = 0.0
    ...
    while true
        ...
        # record the event
        push!(times_log, t)
        revenue += money_made(current_call_durations[i])
        push!(revenue_log,revenue)
        # engage worker in new call and set the call-finish event
        current_call_durations[i] = call_duration()
        event_list[i] = t + current_call_durations[i]
    end
...
function queue_simulator(T, arrival_function, service_function, capacity = Inf, init_queue = 0)
    t = 0.0
    queue_len = init_queue # Length of the queue
...
![Almost Complete Binary Tree](img/tree1.jpg)
...
![Almost Complete Binary Tree in memory](img/tree2.png)
...
package_dir = template("MyPackage")
```

By default this should create a directory structure such as the following in
the `.julia/dev/MyPackage` directory:
...
* Add your new package to the project path using `Pkg.develop("MyPackage")`
* Edit the source code and use Revise.jl to test your changes interactively in the REPL
* Create some tests and run them using `Pkg.test("MyPackage")`

Suppose we created a function `cube_it(x) = x^3` in MyPackage.jl. Tests in
runtests.jl could look something like:
...
We will consider the following (bad!) implementatons of `sum()` as examples of
where type inference fails and benchmarking could be useful.

```{julia}
# Sum where the compiler can only partially infer the type of `s`
function bad_sum(xs)
    s = 0
    for x in xs
        s += x
...
# Sum where the compiler can't infer the type of `s`
z = 0
function really_bad_sum(xs)
   s = z
   for x in xs
       s += x
   end
...
One theme of techniques deals with improving the estimation via generation of alternative random variables instead of $X_1,\ldots,X_n$. This technique sometimes called "change of measure" or [importance sampling](https://en.wikipedia.org/wiki/Importance_sampling). With such a technique a different estimator which still gives unbiased results is used. This alternative estimator can sometimes improve the estimation performance significantly. The key idea is to adjust the estimator (beyond naive Monte Carlo) to take into consideration the different distribution of variables used. If done correctly estimation performance can be significantly improved.
