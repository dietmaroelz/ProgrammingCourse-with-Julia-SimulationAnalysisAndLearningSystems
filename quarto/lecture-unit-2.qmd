...
[![Women coding image](img/women_coders.jpg)](https://cacm.acm.org/news/234820-the-secret-history-of-women-in-coding/fulltext)
...
[![Ada](img/ada.jpg)](https://en.wikipedia.org/wiki/Ada_Lovelace "Ada Lovelace")
...
[![Alan Turing](img/alan-turing.jpg)](https://en.wikipedia.org/wiki/Alan_Turing)
...
## Historical Ladder of delegation in programming (a Faustian bargain)

| Rung | You gain | You give up |
|---|---|---|
| Machine code | Full control over the machine | Impossible to write anything of real complexity |
| Assembly | Human-readable mnemonics, more expressive | Still brutal to write anything complex, tied to one machine |
| Floating point† | Tractable arithmetic on real numbers | Exactness - rounding error, `(a+b)+c ≠ a+(b+c)` |
| Compiled | Source portable, even more expressive | Still low-level - you manage memory yourself |
| Dynamic / interpreted | Machine fully abstracted away, very expressive | Speed |
| JIT | Expressive and fast | Predictability† - performance depends on runtime behaviour in ways that are hard to reason about |
| Parallel / concurrent | Scale beyond one core | Deterministic execution order† - race conditions, results that depend on scheduling |
| Libraries | Enormous leverage, don't reinvent the wheel | Must fit its opinions, lose visibility into what's actually happening |
| Classical ML | Solves problems too complex to hand-specify | Exact specification† - "correct" becomes statistical, not provable |
| LLMs | Extraordinary generality, near-natural-language interface to computation | Specification and implementation control, determinism† - no contract like a compiler gives you, output is plausible, not guaranteed |

† same flavour of bargain each time, trading a guarantee (exact bits, order, correctness) for capability. Floating point started this thread decades before anyone was worried about AI.
