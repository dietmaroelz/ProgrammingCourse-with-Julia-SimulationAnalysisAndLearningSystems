...
4.  A screenshot or copy/paste the output of `test/runtests.jl` at the end of the PDF (this should be run as `julia test/runtests.jl -v` if Task 5 is complete and `julia test/runtests.jl -vp` otherwise).
...
Folders are further subdivided based on the type/functionality they relate to, you should explore these to understand at a high level what each folder/file is for.
...
### Study the Repo
Once you create your own repository, first read the file `README.md`. Then make sure the repository works by running `julia test/runtests.jl -vp` and `julia script/example_script.jl`. At this point simply study the functionality of the repository by looking at the function signatures (not implementations) to understand what the repo can do.
  
### Pretty Printing 
The current `show`/`print` methods do not produce particularly nice outputs. We want polynomials to be rendered similarly to the following (these may not include all the only edge cases):
...
1. At the end of `example_script_task_1.jl`, demonstrate using five hard-coded examples (i.e, explicitly write out the polynomials and do not use `rand` to generate them) of `PolynomialDense{Int}` addition that will overflow by making the coefficients excessively large.
...
1.  Add in all *necessary functions*  (i.e., those for which existing functions defined over the abstract type `Polynomial{C}` are insufficient) in your `polynomial_sparse.jl` defintion file to account for the new underlying representation. For example, `iterate` must be overloaded to account for the fact that the `Heap` is not strictly ordered.
...
3.  Describe (at least) three pros and/or cons (*in a table within your PDF*) between the dense and sparse representations.
...
TODO - Add in points for each task, and run spellchecker. Remove TODO's before pushing. Make template repo. Decide what should go in this final task on bit *$\mathbb{Z}_p[x]$* (quotient idea or just testing things like factorisation and gcd that weren't runnable before to check their mod p works correctly).
