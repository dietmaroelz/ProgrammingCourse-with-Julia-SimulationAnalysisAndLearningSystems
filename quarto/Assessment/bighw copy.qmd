...
Assignment in pairs: **Please email [Mittun](mailto: m.sudhahar@uq.edu.au) ( m.sudhahar@uq.edu.au ) as soon as possible with whom you are forming a pair** to work on the BigHW and submit it together. Send him the names of the two students including their student ids. Just send one email per pair (cc-ing your partner).
...
**IJulia:** We recommend to install [Ijulia](https://github.com/JuliaLang/IJulia.jl) for running Jupyter notebooks in the browser. 
...
**Submission format:**  The submission format is via a GitHub repository in the **MATH25042026 github organisation** as described in Question 9. The submission should include the four submissions forms [I](https://github.com/dietmaroelz/ProgrammingCourse-with-Julia-SimulationAnalysisAndLearningSystems/blob/main/submission_forms_jupyter/Submission-Form-I-Q1-Q4.ipynb), [II](https://github.com/dietmaroelz/ProgrammingCourse-with-Julia-SimulationAnalysisAndLearningSystems/blob/main/submission_forms_jupyter/Submission-Form-II-Q5-Q8.ipynb), [III](https://github.com/dietmaroelz/ProgrammingCourse-with-Julia-SimulationAnalysisAndLearningSystems/blob/main/submission_forms_jupyter/Submission-Form-III-Q9-Q11.ipynb), and [IV](https://github.com/dietmaroelz/ProgrammingCourse-with-Julia-SimulationAnalysisAndLearningSystems/blob/main/submission_forms_jupyter/Submission-Form-IV-Q12-Q15.ipynb). 
...
# Questions 1--4 are submitted on Jupyter [notebook I](https://github.com/dietmaroelz/ProgrammingCourse-with-Julia-SimulationAnalysisAndLearningSystems/blob/main/submission_forms_jupyter/Submission-Form-I-Q1-Q4.ipynb)
...
# Questions 5--8 are submitted on Jupyter [notebook II](https://github.com/dietmaroelz/ProgrammingCourse-with-Julia-SimulationAnalysisAndLearningSystems/blob/main/submission_forms_jupyter/Submission-Form-II-Q5-Q8.ipynb)
...
**7c**: Now investigate the [quick sort algorithm](https://en.wikipedia.org/wiki/Quicksort). As you can see the Wikipedia entry has pseudo-code for the algorithm. Use the help of an LLM (e.g. ChatGPT or Anthropic's Claude) to convert that pseudo-code to Julia code. Then use that code in your function `my_sort_quick()` that uses the quick sort algorithm. Test it on the list of names above, using last name first (it should also support `by=`). Note that the LLM help that you get may not be perfect, so you may need to make adjustments to make it work. 
...
# Questions 9--11 are submitted on Jupyter [notebook III](https://github.com/dietmaroelz/ProgrammingCourse-with-Julia-SimulationAnalysisAndLearningSystems/blob/main/submission_forms_jupyter/Submission-Form-III-Q9-Q11.ipynb)


## Question 9: Setting up your GitHub repo for hand-in.

* Create your GitHub account if you don't have one. Mail your github account to [Mittun](mailto: m.sudhahar@uq.edu.au) ( m.sudhahar@uq.edu.au ). You will then be invited to become a member of the **MATH25042026 github organisation**.
* Within this organisation, create a new repo for this assignment. One student will create the repo, and then invite the paired student to be a collaborator on this project.
* Name the repo exactly as `<<FIRST NAME1>>-<<LAST NAME1>>__<<FIRST NAME2>>-<<LAST NAME2>>2504-2026-BigHW`. 
So for example if your names are "Sam Sample" and "Tania Testname" the repo name should be `Sam-Sample__Tania-Testname-2504-2026-BigHW`.
...
**10a**: Create the folder `hw_q11` and make that your working directory in the shell. Illustrate this via `pwd`.
...
**10h**: Move `my_name2.txt` into `people`, where the file name will be `<<YOUR-NAME>>.txt` with `<<YOUR-NAME>>` being your first and last name without white space using `_`. For this use the `mv` command.

**10i**: Using the same thing you did in **11c**, create three additional files of peoples names in the folder `people`.

**10j**: Change directory into `people`. Now Use `mv` to move one of the people files back into `hw_q11`. Use `..` (double dot).

**10k**: Move another file back into `hw_q11`, but now use an absolute path to describe the destination folder.
...
**10o**: Move back to `hw_q11` and delete the directory `people`. Use either `rm` with a correct flag or `rmdir`.
...
# Questions 12--15 are submitted on Jupyter [notebook IV](https://github.com/dietmaroelz/ProgrammingCourse-with-Julia-SimulationAnalysisAndLearningSystems/blob/main/submission_forms_jupyter/Submission-Form-IV-Q12-Q15.ipynb)
...
**12a**: Consider the functions, $f_1(x) = sin(x^2)$ at $x=1/2$, $f_2(x) = e^{3x^2}$ at $x=1$, and $f_3(x) = atan(2x)/(1+e^{-2x^2})$ at $x=2$. Evaluate the performance of each of the schemes (forward and central) for each of these functions at the given points. In each case plot the error as a function of $h$, and find the optimal $h$. Display your results in a neat manner using one or two plots and a minimal summary.

**12b**: Consider now a vector valued function of the form, $f: {\mathbb R}^K \to {\mathbb R}^K$, defined as,
...
**12c**:  Consider now $z = [1^{1/3}, 2^{1/3}, \ldots, K^{1/3}]^\top$ for $K=10$, $K=20$, and $K=100$. Plot the error of the Jacobian for both the forward difference scheme and the central difference scheme where as error you consider the euclidean distance between the approximated Jacobian and the explicit Jacobian (the square root of the sum up the squares of the differences between entries of the numerical derivative and the explicit one). What is the optimal $h$ in each case?

## Question 13: Markov chains and `LinearAlgebra`
...
## Question 14: Planetary Motion - Part 1
...
Newtons Law states that $m\, a = F(r)$,
...
<!--
## Question 15 (only for MATH7504): Planetary Motion - Part 2
...
**15a**: Implement the Second Order Leapfrog method above for yourself for the second order form of the dynamics of planetary motion. Create similar plots of the Euler and RK4 methods above with $h=0.01$, $h=0.1$, and $h=0.5$.

**15b**: Compare the computational effort of the Leapfrog method with Euler and RK4 methods by comparing how many floating point operations per time step each method uses in your implementation. (This is not a coding question, but a theoretical question).

**15c**: Choose a range to time horizons to simulate over, comment on the trend of the accuracy of each method for long simulation runs. Summarize this analysis in a single compelling plot.

**15d**: The Julia package [DifferentialEquations.jl](https://github.com/SciML/DifferentialEquations.jl) has support for various state of the art techniques, including [symplectic integrators](https://diffeq.sciml.ai/stable/solvers/dynamical_solve/). Choose a state of the art standard ODE solver and a symplectic integrator and use it to find a solution for this ODE system. Compare results with your own implementation of leapfrog and comment on the results.
-->
